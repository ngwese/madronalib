...
int main()
{
	...
	
	{
		...
		std::array<Symbol, 4> dirs{"left", "right", "up", "down"};
		
		ml::NoiseGen r;
		Projection p1(projections::intervalMap({-1, 1}, {0, 4})); // returns [0--3]
		
		for(int i = 0; i < 30; ++i)
		{
			int d = clamp((int)p1(r.getSample()), 0, 3);
			
			Symbol direction = dirs[d];
